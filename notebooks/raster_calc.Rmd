...
eeflux_75_rasters <- list.files(path = here("rasters/ee_growing_season_integrated_brc"), 
                     pattern = ".tif", 
                     full.names = TRUE)
...
chirps_75_rasters <- list.files(path = here("rasters/ee_season_precip_data_brc"), 
                     pattern = ".tif", 
                     full.names = TRUE)
...
slope <- calc(s, fun)
writeRaster(x = slope,
              filename = here("rasters/raster_download/slope_125_275_brc.tif"),
              format = 'GTiff',
            overwrite = TRUE)
...
writeRaster(x = r.squared,
              filename = here("rasters/raster_download/r_squared_125_275_brc.tif"),
              format = 'GTiff',
            overwrite = TRUE)
...
p.val <- calc(s, fun)
writeRaster(x = p.val,
              filename = here("rasters/raster_download/p_val_125_275_brc.tif"),
              format = 'GTiff',
            overwrite = TRUE)

#Try a seasonal regression - April/May/June
